...
int CheckBoards(std::vector<Board> &boards) {
	int ret = -1;
	for (int b = 0; b < boards.size(); b++) {
		if (boards[b].hasWon) continue;
		// Check rows
		for (int i = 0; i < 5; i++) {
			int j = 0;
...
int main(int argc, char **argv) {
	...
	for (int num : numsToDraw) {
		...
		if (winnerIndex > -1) {
			...
			std::cout << "Final score: " << boards[winnerIndex].UnmarkedSum() * num << std::endl << std::endl;
			lastWinnerIndex = winnerIndex;
			lastWinnerNum = num;
		}
	}

	std::cout << "Board " << lastWinnerIndex << " is the last winner!\n";
	std::cout << "Last number to be drawn: " << lastWinnerNum << std::endl;
	std::cout << "Final score: " << boards[lastWinnerIndex].UnmarkedSum() * lastWinnerNum << std::endl;
